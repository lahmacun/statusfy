...
- (void)setStatusItemTitle
{
    ...
    if (trackName && artistName) {
        ...
        NSAppleScript* getCurrentVolume = [[NSAppleScript alloc] initWithSource:@"get output volume of (get volume settings)"];
...
//            
//            if(!muteSpotifyScript)
//                muteSpotifyScript = [[NSAppleScript alloc] initWithSource:
//                                        @"tell application \"Spotify\" set audio volume 0"];
////            NSString *statusString = [[muteSpotifyScript executeAndReturnError:nil] stringValue];
            
        } else {
            if ([currentVolume  isEqual: @"0"]) {
//                NSAppleScript* as = [[NSAppleScript alloc] initWithSource:[@"set volume " stringByAppendingString: currentVolume]];
                NSAppleScript* as = [[NSAppleScript alloc] initWithSource:@"set volume 100"];
                [as executeAndReturnError:nil];
            }
        }
